...
import (
	...
	"github.com/pirsch-analytics/pirsch/v6/_pkg/db"
...
func TestAnalyzer_EventsPathRegex(t *testing.T) {
	db.CleanupDB(t, dbClient)
	saveSessions(t, [][]model.Session{
		{
			{Sign: 1, VisitorID: 1, Time: util.Today(), Start: time.Now(), EntryPath: "/", ExitPath: "/exit", PageViews: 2},
			{Sign: 1, VisitorID: 2, Time: util.Today(), Start: time.Now(), EntryPath: "/", ExitPath: "/", PageViews: 1},
			{Sign: 1, VisitorID: 3, Time: util.Today(), Start: time.Now(), EntryPath: "/foo", ExitPath: "/foo", PageViews: 1},
		},
	})
	assert.NoError(t, dbClient.SavePageViews([]model.PageView{
		{VisitorID: 1, Time: util.Today(), Path: "/"},
		{VisitorID: 1, Time: util.Today(), Path: "/exit"},
		{VisitorID: 2, Time: util.Today(), Path: "/"},
		{VisitorID: 3, Time: util.Today(), Path: "/foo"},
	}))
	assert.NoError(t, dbClient.SaveEvents([]model.Event{
		{Name: "event", DurationSeconds: 5, VisitorID: 1, Time: util.Today(), Path: "/"},
		{Name: "event", DurationSeconds: 8, VisitorID: 2, Time: util.Today(), Path: "/"},
		{Name: "event", DurationSeconds: 3, VisitorID: 3, Time: util.Today(), Path: "/foo"},
	}))
	time.Sleep(time.Millisecond * 100)
	analyzer := NewAnalyzer(dbClient)
	stats, err := analyzer.Events.Events(nil)
