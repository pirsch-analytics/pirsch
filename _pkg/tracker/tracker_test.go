...
import (
	...
	"github.com/pirsch-analytics/pirsch/v6/_pkg/analyzer"
	"github.com/pirsch-analytics/pirsch/v6/_pkg/db"
	"github.com/pirsch-analytics/pirsch/v6/_pkg/model"
	"github.com/pirsch-analytics/pirsch/v6/_pkg/tracker/geodb"
	"github.com/pirsch-analytics/pirsch/v6/_pkg/tracker/ip"
	"github.com/pirsch-analytics/pirsch/v6/_pkg/tracker/session"
...
func TestTracker_PageViewBounce(t *testing.T) {
	...
	tracker.PageView(req, 123, Options{})
	time.Sleep(time.Second)
	tracker.Event(req, 123, EventOptions{Name: "event"}, Options{})
	time.Sleep(time.Second)
	tracker.PageView(req, 123, Options{})
	tracker.Flush()
	sessions := client.GetSessions()
	...
	assert.Equal(t, uint32(0), sessions[0].DurationSeconds)
	assert.Equal(t, uint32(1), sessions[2].DurationSeconds)
	assert.Equal(t, uint32(2), sessions[4].DurationSeconds)
	assert.True(t, sessions[0].IsBounce)
}
