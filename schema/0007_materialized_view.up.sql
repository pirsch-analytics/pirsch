...
ALTER TABLE event DROP COLUMN event_duration_seconds;

CREATE MATERIALIZED VIEW sessions
ENGINE = AggregatingMergeTree()
PARTITION BY toYYYYMM(time)
ORDER BY (client_id, visitor_id, session_id, time, path, title)
AS
    SELECT client_id, visitor_id, session_id, path, title, time, duration_seconds,
    ...
    FROM (
        ...
        FROM (
            ...
            FROM (
                SELECT * FROM hit ORDER BY client_id, visitor_id, session_id, time
            )
            GROUP BY client_id, visitor_id, session_id
        )
        GROUP BY client_id, visitor_id, session_id, path_title, path, title, time,
...
ORDER BY (client_id, visitor_id, session_id, time, path, title)
AS
    SELECT client_id, visitor_id, session_id, event_name, event_meta_keys, event_meta_values, path, title, time,
    ...
    FROM (
        ...
        FROM (
            ...
            argMax(utm_term, time) utm_term
            FROM (
                SELECT * FROM event ORDER BY client_id, visitor_id, session_id, time
            )
            GROUP BY client_id, visitor_id, session_id, time, event_name, event_meta_keys, event_meta_values
        )
        GROUP BY client_id, visitor_id, session_id, event_name, event_meta_keys, event_meta_values, path_title, path, title, time,
        duration_seconds, language, country_code, city, referrer, referrer_name, referrer_icon,
        os, os_version, browser, browser_version, desktop, mobile, screen_width, screen_height, screen_class,
        utm_source, utm_medium, utm_campaign, utm_content, utm_term
    )
