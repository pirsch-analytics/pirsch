...
func CleanupDB(t *testing.T, client *Client) {
	...
	assert.NoError(t, err)
	_, err = client.Exec(`ALTER TABLE "request" DELETE WHERE 1=1`)
	assert.NoError(t, err)
...
func DropDB(t *testing.T, client *Client) {
	...
	assert.NoError(t, err)
	_, err = client.Exec(`DROP TABLE IF EXISTS "event_new"`)
	assert.NoError(t, err)
	_, err = client.Exec(`DROP TABLE IF EXISTS "event_backup"`)
	assert.NoError(t, err)
	_, err = client.Exec(`DROP TABLE IF EXISTS "request"`)
	assert.NoError(t, err)
