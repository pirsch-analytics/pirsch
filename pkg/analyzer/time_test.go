...
func TestAnalyzer_AvgTimeOnPage(t *testing.T) {
	...
	time.Sleep(time.Millisecond * 20)
	analyzer := NewAnalyzer(dbClient)
	byDay, err := analyzer.Time.AvgTimeOnPage(&Filter{From: util.PastDay(3), To: util.Today()})
	assert.NoError(t, err)
	...
	byDay, err = analyzer.Time.AvgTimeOnPage(&Filter{PathPattern: []string{"\\/.*"}, From: util.PastDay(3), To: util.Today()})
	assert.NoError(t, err)
	assert.Len(t, byDay, 4)
	assert.Equal(t, 8, byDay[0].AverageTimeSpentSeconds)
	assert.Equal(t, 4, byDay[1].AverageTimeSpentSeconds)
