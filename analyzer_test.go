...
func TestAnalyzer_PageReferrer(t *testing.T) {
	...
	for _, tenantID := range tenantIDs {
		...
		if visitors[0].Referrer.String != "ref2" || visitors[0].ReferrerName.String != "ref2Name" || visitors[0].ReferrerIcon.String != "ref2Icon" || visitors[0].Visitors != 43 || !inRange(visitors[0].RelativeVisitors, 0.977) ||
			visitors[1].Referrer.String != "ref1" || visitors[1].ReferrerName.Valid || visitors[1].ReferrerIcon.Valid || visitors[1].Visitors != 1 || !inRange(visitors[1].RelativeVisitors, 0.022) {
			t.Fatalf("Visitors not as expected: %v", visitors)
		}
	}
}
