...
func TestAnalyzer_VisitorsAvgSessionDurationAvgTimeOnPagePeriod(t *testing.T) {
	...
	assert.Equal(t, week, visitors[0].Week)
	assert.Equal(t, week+1, visitors[1].Week)
	assert.Equal(t, week+2, visitors[2].Week)
	assert.Equal(t, week+3, visitors[3].Week)
	...
	assert.NoError(t, err)
	assert.Len(t, asd, 4)
	assert.Equal(t, week, asd[0].Week)
	assert.Equal(t, week+1, asd[1].Week)
	assert.Equal(t, week+2, asd[2].Week)
	assert.Equal(t, week+3, asd[3].Week)
	assert.Equal(t, 12, asd[0].AverageTimeSpentSeconds)
	assert.Equal(t, 0, asd[1].AverageTimeSpentSeconds)
	assert.Equal(t, 29, asd[2].AverageTimeSpentSeconds)
	assert.Equal(t, 34, asd[3].AverageTimeSpentSeconds)
	atop, err := analyzer.AvgTimeOnPage(&Filter{From: pastDay(30), To: Today(), Period: PeriodWeek})
	assert.NoError(t, err)
	assert.Len(t, atop, 4)
	assert.Equal(t, week, atop[0].Week)
	assert.Equal(t, week+1, atop[1].Week)
	assert.Equal(t, week+2, atop[2].Week)
	assert.Equal(t, week+3, atop[3].Week)
	assert.Equal(t, 7, atop[0].AverageTimeSpentSeconds)
	assert.Equal(t, 0, atop[1].AverageTimeSpentSeconds)
	assert.Equal(t, 3, atop[2].AverageTimeSpentSeconds)
