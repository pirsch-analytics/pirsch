...
func TestAnalyzer_VisitorsAvgSessionDurationAvgTimeOnPagePeriod(t *testing.T) {
	...
	analyzer := NewAnalyzer(dbClient, nil)
	visitors, err := analyzer.Visitors(&Filter{From: pastDay(30), To: Today(), Period: PeriodWeek})
	assert.NoError(t, err)
	assert.Len(t, visitors, 5)
	assert.Equal(t, pastWeek(5), getWeek(visitors[0].Week))
	assert.Equal(t, pastWeek(4), getWeek(visitors[1].Week))
	assert.Equal(t, pastWeek(3), getWeek(visitors[2].Week))
	assert.Equal(t, pastWeek(2), getWeek(visitors[3].Week))
	assert.Equal(t, pastWeek(1), getWeek(visitors[4].Week))
	assert.Equal(t, 1, visitors[0].Visitors)
	assert.Equal(t, 0, visitors[1].Visitors)
	assert.Equal(t, 0, visitors[2].Visitors)
	assert.Equal(t, 1, visitors[3].Visitors)
	assert.Equal(t, 1, visitors[4].Visitors)
	asd, err := analyzer.AvgSessionDuration(&Filter{From: pastDay(30), To: Today(), Period: PeriodWeek})
	assert.NoError(t, err)
	assert.Len(t, asd, 5)
	assert.Equal(t, pastWeek(5), getWeek(asd[0].Week))
	assert.Equal(t, pastWeek(4), getWeek(asd[1].Week))
	assert.Equal(t, pastWeek(3), getWeek(asd[2].Week))
	assert.Equal(t, pastWeek(2), getWeek(asd[3].Week))
	assert.Equal(t, pastWeek(1), getWeek(asd[4].Week))
	assert.Equal(t, 3, asd[0].AverageTimeSpentSeconds)
	assert.Equal(t, 0, asd[1].AverageTimeSpentSeconds)
	assert.Equal(t, 0, asd[2].AverageTimeSpentSeconds)
	assert.Equal(t, 4, asd[3].AverageTimeSpentSeconds)
	assert.Equal(t, 6, asd[4].AverageTimeSpentSeconds)
	atop, err := analyzer.AvgTimeOnPage(&Filter{From: pastDay(30), To: Today(), Period: PeriodWeek})
	assert.NoError(t, err)
	assert.Len(t, atop, 5)
	assert.Equal(t, pastWeek(5), getWeek(atop[0].Week))
	assert.Equal(t, pastWeek(4), getWeek(atop[1].Week))
	assert.Equal(t, pastWeek(3), getWeek(atop[2].Week))
	assert.Equal(t, pastWeek(2), getWeek(atop[3].Week))
	assert.Equal(t, pastWeek(1), getWeek(atop[4].Week))
	assert.Equal(t, 2, atop[0].AverageTimeSpentSeconds)
	...
	assert.Equal(t, 0, atop[4].AverageTimeSpentSeconds)
	visitors, err = analyzer.Visitors(&Filter{From: pastDay(91), To: Today(), Period: PeriodMonth})
	assert.NoError(t, err)
	assert.Len(t, visitors, 5)
	assert.Equal(t, pastMonth(4), visitors[0].Month)
	assert.Equal(t, pastMonth(3), visitors[1].Month)
	assert.Equal(t, pastMonth(2), visitors[2].Month)
	assert.Equal(t, pastMonth(1), visitors[3].Month)
	assert.Equal(t, pastMonth(0), visitors[4].Month)
	assert.Equal(t, 0, visitors[0].Visitors)
	assert.Equal(t, 0, visitors[1].Visitors)
	assert.Equal(t, 0, visitors[2].Visitors)
	assert.Equal(t, 3, visitors[3].Visitors)
	assert.Equal(t, 0, visitors[4].Visitors)
	asd, err = analyzer.AvgSessionDuration(&Filter{From: pastDay(91), To: Today(), Period: PeriodMonth})
	assert.NoError(t, err)
	assert.Len(t, asd, 5)
	assert.Equal(t, pastMonth(4), asd[0].Month)
	assert.Equal(t, pastMonth(3), asd[1].Month)
	assert.Equal(t, pastMonth(2), asd[2].Month)
	assert.Equal(t, pastMonth(1), asd[3].Month)
	assert.Equal(t, pastMonth(0), asd[4].Month)
	assert.Equal(t, 0, asd[0].AverageTimeSpentSeconds)
	assert.Equal(t, 0, asd[1].AverageTimeSpentSeconds)
	assert.Equal(t, 0, asd[2].AverageTimeSpentSeconds)
	assert.Equal(t, 2, asd[3].AverageTimeSpentSeconds)
	assert.Equal(t, 0, asd[4].AverageTimeSpentSeconds)
	atop, err = analyzer.AvgTimeOnPage(&Filter{From: pastDay(91), To: Today(), Period: PeriodMonth})
	assert.NoError(t, err)
	assert.Len(t, atop, 5)
	assert.Equal(t, pastMonth(4), atop[0].Month)
	assert.Equal(t, pastMonth(3), atop[1].Month)
	assert.Equal(t, pastMonth(2), atop[2].Month)
	assert.Equal(t, pastMonth(1), atop[3].Month)
	assert.Equal(t, pastMonth(0), atop[4].Month)
	...
	assert.Equal(t, year-1, visitors[0].Year.Year())
	assert.Equal(t, year, visitors[1].Year.Year())
	assert.Equal(t, 1, visitors[0].Visitors)
	...
	assert.NoError(t, err)
	assert.Len(t, asd, 2)
	assert.Equal(t, year-1, asd[0].Year.Year())
	assert.Equal(t, year, asd[1].Year.Year())
	assert.Equal(t, 0, asd[0].AverageTimeSpentSeconds)
